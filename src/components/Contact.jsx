...
        setStatus('Success! Message sent.'); // Pesan sukses yang muncul di web
        form.current.reset(); // Form otomatis kosong lagi
    }, (error) => {
...
              <button 
              ...
              {status && (
                <p className={`text-center mt-4 font-bold ${status.includes('Success') ? 'text-green-400' : 'text-red-400'}`}>
                  {status}
                </p>
              )}
            </form>
