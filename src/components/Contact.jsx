  ...
  const renderBrandIcon = (brandName, color = "currentColor") => {
    ...
    return (
      <img 
        ...
        // Style ini memastikan warna SVG bisa mengikuti warna teks Tailwind
        style={{ filter: color === "currentColor" ? 'brightness(0) invert(1)' : 'none' }}
      />
    );
  };

  // Fungsi pembantu untuk ikon Lucide klasik (Email, Phone, Location)
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-28">
          ...
          <p className="text-gray-400 mt-4 text-lg max-w-2xl mx-auto leading-relaxed">
            ...
            Feel free to reach out, let’s bring ideas to life together.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-12">
            <div>
              ...
              <div className="space-y-8">
                ...
                {/* Email (Pake Lucide Aman) */}
                <div className="flex items-center gap-6 group">
                  <div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center p-3.5 group-hover:bg-(--color-neon-purple)/20 transition-all text-(--color-neon-purple)">
                    {renderLucideIcon(Icon.Mail, "📧")}
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 uppercase font-bold">Email</p>
                    <p className="text-lg text-gray-200">annisayusri59@gmail.com</p>
                  </div>
                </div>

                {/* Phone (Pake Lucide Aman) */}
                <div className="flex items-center gap-6 group">
                  <div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center p-3.5 group-hover:bg-(--color-neon-purple)/20 transition-all text-(--color-neon-purple)">
                  ...
                  <div>
                    ...
                    <p className="text-lg text-gray-200">+62 896 3587 6114</p>
                  </div>
                </div>

                {/* Location (Pake Lucide Aman) */}
                <div className="flex items-center gap-6 group">
                  ...
                  <div>
                    ...
                    <p className="text-lg text-gray-200">Semarang, Jawa Tengah, Indonesia</p>
                  </div>
                </div>
            ...
            {/* === SOCIAL MEDIA CONNECT (DENGAN PERBAIKAN POSISI) === */}
            <div className="pt-10 border-t border-white/5 text-center lg:text-left">
              <p className="text-xl font-bold mb-10 text-gray-400 uppercase tracking-[0.25em] lg:pl-30 block w-full">
                Connect With Me
              </p>
              
              {/* --- INI KUNCI GESER KE TENGAH --- */}
              {/* Kita hapus 'lg:justify-start' dan ganti jadi 'lg:justify-center' atau 
                  pake padding biar geser proporsional. Di sini aku pake justify-center 
                  dan tambah max-w agar terlihat rapi di tengah containernya. */}
              <div className="flex flex-wrap gap-6 justify-center lg:justify-start lg:pl-20">
          ...
          {/* Sisi Kanan: Form (Tetap) */}
          <div className="bg-white/[0.03] backdrop-blur-xl p-10 rounded-[40px] border border-white/5 shadow-2xl">
            <h4 className="text-2xl font-bold mb-8 text-center text-white">Send a Message</h4>
            <form className="space-y-6">
              <input type="text" placeholder="What's your name?" className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-white outline-none focus:border-(--color-neon-purple) transition-all" />
              <input type="email" placeholder="Write your E-mail here..." className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-white outline-none focus:border-(--color-neon-blue) transition-all" />
              <textarea rows="4" placeholder="Tell me about your idea..." className="w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-white outline-none focus:border-(--color-neon-purple) transition-all resize-none"></textarea>
              <button className="w-full py-5 bg-gradient-to-r from-(--color-neon-purple) to-(--color-neon-blue) text-white font-black text-lg rounded-2xl hover:shadow-[0_0_20px_rgba(168,85,247,0.3)] transition-all flex items-center justify-center gap-2">
                Send Message {Icon.Send && <Icon.Send className="w-5 h-5" />}
              </button>
            </form>
          </div>
